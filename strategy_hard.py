...
for j in range(N):
    ...
    for cur_epokh in range(0, epoch_count):
        ...
        winner_prize = reserv_resources * prize_koef
        #winner_prize = initial_resources * prize_koef
        # print(winner_prize)
        intervals = generate_intervals(nodes_count - 1, stack_distribution.copy(), algorithm)
        ...
        for i in range(len(winners)):
            stack_index[i] += (1 - komis)*(winner_prize / slot_count * winners[i]) * ((stack_index[i]) / (stack_distribution[i]))
            stack_distribution[i] += komis*(winner_prize / slot_count * winners[i]) * ((stack_index[i]) / (stack_distribution[i]))
            stack_distribution[i] += (winner_prize / slot_count * winners[i]) * ((stack_distribution[i] - stack_index[i]) / (stack_distribution[i]))
            # print(((winner_prize / slot_count * winners[i]) * ((stack_index[i]) / (stack_distribution[i] - stack_index[i]))) + (winner_prize / slot_count * winners[i]) * ((stack_distribution[i] - 2*stack_index[i]) / (stack_distribution[i] - stack_index[i])))
            # print(winner_prize / slot_count * winners[i])
    ...
    stack_distribution = absolute_to_relative_stack(len(stack_distribution), stack_distribution)
