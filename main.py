...
for cur_epokh in range(0, epoch_count):
    ...
    loser_prob.append(capture_winning_prob(nodes_count, stack_distribution.copy(), loser_index, algorithm))
...
plt.title('Динамика изменения вероятности выигрыша конкретного(' + str(winning_index + 1) + '-ого) пользователя')
...
plt.title('Динамика изменения вероятности выигрыша конкретного(' + str(loser_index + 1) + '-ого) пользователя')
plt.xlabel('Номер эпохи')
plt.ylabel('Вероятность выигрыша')
plt.show()
...
plt.title('Зависимость вероятности выигрыша от номера узла после ' + str(epoch_count) + ' эпох')
plt.xlabel('Номер узла')
plt.ylabel('Вероятность выигрыша')
plt.show()
